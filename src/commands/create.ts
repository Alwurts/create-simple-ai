...
async function gatherConfiguration(projectName?: string, options: CLIOptions): Promise<ProjectConfig> {
  ...
  const config: ProjectConfig = {
    ...
    database: options.database || (options.yes ? 'sqlite' : await getDatabaseChoice()),
    auth: options.auth || (options.yes ? 'better-auth' : await getAuthChoice()),
    examples: options.examples || (options.yes ? [] : await getExampleChoices()),
