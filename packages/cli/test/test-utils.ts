...
export interface TestConfig extends Omit<CLIOptions, "packageManager" | "database"> {
  projectName: string;
  expectError?: boolean;
 ...
 */
export async function validateDatabaseConfig(projectDir: string): Promise<void> {
 ...
 */
export async function validatePackageJsonDependencies(projectDir: string): Promise<void> {
